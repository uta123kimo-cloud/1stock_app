...
def backtest_all_trades(df):
    ...
    for i in range(len(df)):
        ...
        if in_trade:
            ...
            # 達標紀錄
            if reach_10 is None and ret >= 10: reach_10 = days
            if reach_20 is None and ret >= 20: reach_20 = days
            if reach_m10 is None and ret <= -10: reach_m10 = days
            ...
            if "空單進場" in status or sz < -1:
                ...
                observe_count += 1
                if observe_count >= 5: exit_flag = True
            else:
                observe_count = 0
...
if run_btn:
    ...
    for col, (name, sym) in zip(cols, INDEX_LIST.items()):
        ...
        status, _ = map_status(op, sz)
        def arrow(v,p):
            if pd.isna(v) or pd.isna(p): return "未提供"
            if v>p: return "↑"
            if v<p: return "↓"
            return "→"
        price = round(curr["Close"],0) if ".TW" in sym else round(curr["Close"],2)
        pvo_str = f"{curr['PVO']:.2f}" if not pd.isna(curr['PVO']) else "未提供"
        vri_str = f"{curr['VRI']:.2f}" if not pd.isna(curr['VRI']) else "未提供"
        col.markdown(f"""
        **{name}**  
        收盤：{price}  
        狀態：{status}  
    ...
    if mode=="單股分析":
        ...
        if df is None or len(df)<50:
            ...
            col2.metric("PVO", f"{curr['PVO']:.2f}" if not pd.isna(curr['PVO']) else "未提供")
            col3.metric("VRI", f"{curr['VRI']:.2f}" if not pd.isna(curr['VRI']) else "未提供")
